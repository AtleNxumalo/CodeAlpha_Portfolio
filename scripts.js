// WONDERING WHERE THE PROJECTS CAME FROM? HERE!
...
// Filters container for category buttons
const filtersEl = document.querySelector('.filters');

// Grid/container where project cards will be displayed
const gridEl = document.getElementById('projectGrid');

// Modal elements for project details view
const modal = document.getElementById('modal');               // Overlay for modal
const modalTitle = document.getElementById('modalTitle');     // Project title inside modal
const modalDescription = document.getElementById('modalDescription'); // Project description inside modal
const modalImages = document.getElementById('modalImages');   // Container for carousel images/videos
...
// Create unique categories from projects + add "all" option
const categories = ['all', ...new Set(projects.map(p => p.category))];

// Track which category is active (default = all)
...
function renderFilters() {
    ...
    // Loop through categories and create buttons dynamically
    categories.forEach(cat => {
        ...
        // On click, update active category and re-render projects
        btn.onclick = () => {
            activeCategory = cat;
            renderProjects();
        };
...
function renderProjects() {
    // Always render filters first (so active class updates)
    renderFilters();

    // Clear previous projects
    gridEl.innerHTML = '';

    // Filter projects by active category (or show all)
    projects
        .filter(p => activeCategory === 'all' || p.category === activeCategory)
        .forEach((p, index) => {
            // Alternate thumbnail images for visual variety
            ...
            // Set inner HTML for card (thumbnail, title, description, tags)
            card.innerHTML = `
            ...
            // On click, open modal with project details
...
function openModal(project) {
    ...
    modalImages.innerHTML = `
        ...
        if (media.endsWith('.mp4')) {
            // Use video tag if media is a .mp4 file
            return `
                        <video
                            src="${media}"
                            controls
    ...
    // Hide carousel arrows if only one slide
    prevBtn.style.display = total > 1 ? 'block' : 'none';
    nextBtn.style.display = total > 1 ? 'block' : 'none';

    // Pause all videos in carousel (used when switching slides)
    ...
    function updateSlide() {
        ...
        // Play only current video slide, pause others
    ...
    prevBtn.onclick = () => {
        pauseAllVideos();
        currentSlide = (currentSlide - 1 + total) % total; // wrap around
        updateSlide();
    };

    nextBtn.onclick = () => {
        pauseAllVideos();
        currentSlide = (currentSlide + 1) % total; // wrap around
        updateSlide();
    };
